...
blob = BlobStorage(blob_credentials)
...
@app.route('/add_event/<id>', methods=['GET'])
...
def subscribe():
    ...
    print(f'{request.url_root}add_event/{advert_id}')
    print(requests.get('https://petifyfunction.azurewebsites.net/api/my-func', data=json.dumps(data)))
    print(requests.get(f'{request.url_root}add_event/{advert_id}'))

    return redirect(f'{request.url_root}add_event/{advert_id}')
