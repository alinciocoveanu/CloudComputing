...
def load_credentials(token_name):
    credentials_pickle_file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), token_name)
    return pickle.load(open(credentials_pickle_file_path, 'rb'))


def add_event(advert, token_name):
    global credentials
    # print(advert)
    credentials = Credentials(load_credentials(token_name)['access_token'])
