...
def login():
    ...
    request_uri = client.prepare_request_uri(authorization_endpoint,
                                             redirect_uri=request.base_url.replace('http', 'https').replace('httpss',
                                                                                                            'https') + "/callback",
    ...
    try:
        ...
        token_url, headers, body = client.prepare_token_request(token_endpoint,
                                                                ...
                                                                redirect_url=request.base_url.replace('http',
                                                                                                      'https').replace(
                                                                    'httpss', 'https'), code=code)
        token = requests.post(token_url, headers=headers, data=body,
                              auth=(GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET)).json()
        ...
        token_name = os.path.join(path_to_project, ('tokens/token' + users_email.split('@')[0]).replace('.', '') + '.pickle').replace('\\', '/')
        print(token_name)
        with open(token_name, 'wb') as f:
            pickle.dump(token, f)

        if not db.get_user_by_id():
            send_welcome_message('cloudcomputingteamfii@gmail.com', users_email, users_name)
            db.add_user()
